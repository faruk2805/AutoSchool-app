// components/LekcijaRaskrsnice.tsx
...
export default function LekcijaRaskrsnice() {
  ...
  useEffect(() => {
    console.log('📖 Učitavam lekciju raskrsnica:', lessonNumber);
    loadUserData();
    loadQuestions();
  }, [lessonNumber]);
  ...
  const loadQuestions = async () => {
    try {
      console.log('📚 Učitavam pitanja za lekciju raskrsnica:', lessonNumber);
      setIsLoading(true);
      ...
      console.log('✅ Učitano svih pitanja raskrsnica:', allQuestions.length);
      ...
      console.log(`📖 Lekcija raskrsnica ${lessonNumber}: pitanja ${startIndex + 1}-${endIndex}`);
  ...
  const sendResultsToBackend = async (correctCount: number, totalQuestions: number, score: number): Promise<boolean> => {
    try {
      ...
      console.log('📤 Šaljem rezultate raskrsnica na backend:', testResult);
      ...
      console.log('✅ Rezultati raskrsnica uspješno poslani:', result);
      return true;

    } catch (error) {
      console.error('🚨 Greška pri slanju rezultata raskrsnica:', error);
      return false;
    }
  };
  ...
  const saveLessonProgress = async (correctCount: number, totalQuestions: number, score: number, passed: boolean) => {
    try {
      const progressData = await AsyncStorage.getItem('raskrsnicaProgress');
      ...
      await AsyncStorage.setItem('raskrsnicaProgress', JSON.stringify(updatedProgress));
      console.log('💾 Progres lekcije raskrsnica sačuvan');
  ...
  const calculateResults = async () => {
    ...
    console.log('📊 Rezultat raskrsnica:', { correctCount, total: questions.length, score, passed });
  ...
  const getLessonTitle = (lessonNumber: number) => {
    const titles = {
      1: 'Osnovne Raskrsnice',
      2: 'Pravilo Desne Ruke',
      3: 'Raskrsnice sa Prednošću',
      4: 'Raskrsnice sa Tramvajem',
      5: 'Kružni Tok',
      6: 'Složene Raskrsnice',
      7: 'Raskrsnice sa Signalima',
      8: 'Specijalne Situacije'
    };
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.header}>
          <TouchableOpacity onPress={() => router.push('/raskrsnice/lekcije')} style={styles.backButton}>
