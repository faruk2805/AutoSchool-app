...
export default function NasumicniTestRaskrsnice() {
  ...
  const loadUserStats = async (user_Id: string) => {
    try {
      ...
      const raskrsnicaResults = userResults.rezultati.filter(result => 
        result.tip === 'raskrsnica'
      );

      console.log('🚦 Rezultati raskrsnica:', raskrsnicaResults.length, 'testova');

      let completedTests = raskrsnicaResults.length;
      ...
      raskrsnicaResults.forEach(result => {
      ...
      console.log('📈 Nova statistika raskrsnica:', newStats);
      setUserStats(newStats);

    } catch (error) {
      console.log('🚨 Greška pri učitavanju statistike raskrsnica:', error);
      setUserStats({
        completedTests: 0,
        correctAnswers: 0,
        totalQuestions: 0,
  ...
  const loadRandomQuestions = async () => {
    try {
      console.log('🔍 Učitavam pitanja za raskrsnice sa APIja...');
      setIsLoading(true);
      setLoadError(false);
      
      const response = await fetch(`${API_BASE_URL}/api/questions/tip/raskrsnica`);
      ...
      const raskrsnicaQuestions = await response.json();
      console.log('✅ Učitano raskrsnica pitanja:', raskrsnicaQuestions.length);
      
      if (!Array.isArray(raskrsnicaQuestions)) {
        throw new Error('API nije vratio listu pitanja');
      }

      if (raskrsnicaQuestions.length === 0) {
        throw new Error('Nema pitanja za raskrsnice na serveru');
      }

      const shuffledQuestions = [...raskrsnicaQuestions]
        .sort(() => 0.5 - Math.random());
      
      const selectedQuestions = shuffledQuestions.slice(0, 10);
      
      console.log('🎯 Odabrano nasumičnih pitanja za raskrsnice:', selectedQuestions.length);
      setQuestions(selectedQuestions);
      
    } catch (error) {
      console.error('🚨 Greška pri učitavanju pitanja raskrsnica:', error);
      setLoadError(true);
      setQuestions([]);
    } finally {
      setIsLoading(false);
  ...
  const startTest = () => {
    ...
    console.log('🚀 Pokrećem NASUMIČNI test RASKISSNICA sa pitanjima:', questions.length);
    
    router.push({
      pathname: "/raskrsnice/test-screen",
      params: {
        questions: JSON.stringify(questions),
        testType: 'nasumicni-raskrsnice',
        testMode: 'nasumicni',
        userId: userId
      }
    });
  ...
  const handleBackToTests = () => {
    ...
    // Vrati korisnika na nasumične testove raskrsnica
  ...
  return (
    <View style={styles.container}>
      ...
      >
        <TouchableOpacity onPress={() => router.push('/raskrsnice')} style={styles.backButton}>
          <Ionicons name="chevron-back" size={28} color="#FFF" />
        </TouchableOpacity>
        <View style={styles.headerCenter}>
          <Text style={styles.headerTitle}>Nasumični Test</Text>
          <Text style={styles.headerSubtitle}>Raskrsnice • Random pitanja</Text>
        </View>
      ...
      >
        ...
        >
          ...
          <View style={styles.infoCard}>
            ...
            <View style={styles.infoItem}>
              <LinearGradient
                colors={['#8104FA', '#6A00FF']}
                style={styles.infoIcon}
              >
                <Ionicons name="git-merge" size={20} color="#FFF" />
              </LinearGradient>
              <View style={styles.infoText}>
                <Text style={styles.infoTitle}>Raskrsnice i Prvenstvo Prolaza</Text>
                <Text style={styles.infoDescription}>Sva pitanja su iz oblasti raskrsnica, pravila prvenstva prolaza i sigurnog prolaženja</Text>
              </View>
            </View>
            ...
            <View style={styles.infoItem}>
              ...
              <View style={styles.infoText}>
                <Text style={styles.infoTitle}>20 Minuta</Text>
          ...
          <View style={styles.motivationCard}>
            ...
            <Text style={styles.motivationText}>
              {userStats.completedTests > 0 
                ...
                : 'Ovo je tvoja prva prilika da pokažeš svoje znanje o raskrsnicama i pravilima prvenstva prolaza!'
              }
            </Text>
          </View>
        </Animated.View>
      ...
      >
        ...
        >
          <LinearGradient
            colors={questions.length === 0 ? ['#9CA3AF', '#6B7280'] : ['#FF6B6B', '#FF8E53']}
          ...
          <Animated.View style={[
            ...
            <View style={styles.modalBody}>
              ...
              <Text style={styles.modalMessage}>
                {testResults?.passed 
                  ? 'Odlično ste savladali test o raskrsnicama! Nastavite sa vježbanjem.'
