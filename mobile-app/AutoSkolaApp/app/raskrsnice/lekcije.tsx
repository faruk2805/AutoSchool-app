...
export default function LekcijeRaskrsnice() {
  ...
  const loadProgressFromAPI = async (user_Id: string) => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/testresults/user/${user_Id}/raskrsnica/lekcijski`
      );
      
      console.log('🌐 API URL:', `${API_BASE_URL}/api/testresults/user/${user_Id}/raskrsnica/lekcijski`);
      ...
      // FEJK PROGRESS - prva lekcija dostupna, ostale zaključane
      const initialProgress = Array.from({ length: totalLessons }, (_, i) => ({
        lesson: i + 1,
        completed: i === 0, // Samo prva lekcija završena za demonstraciju
        score: i === 0 ? 85 : 0, // Prva lekcija ima 85% za demonstraciju
        correctAnswers: i === 0 ? 8 : 0,
        totalQuestions: questionsPerLesson,
        dateCompleted: i === 0 ? new Date().toISOString() : undefined
      }));
      setLessonsProgress(initialProgress);
    } finally {
      setIsLoading(false);
  ...
  const getLessonDescription = (lessonNumber: number) => {
    const descriptions = {
      1: 'Osnovna pravila prolaska kroz jednostavne raskrsnice',
      2: 'Primjena pravila desne ruke u različitim situacijama',
      3: 'Raskrsnice sa oznakama prvenstva prolaza',
      4: 'Prolazak kroz raskrsnice sa tramvajskim linijama',
      5: 'Pravila za kružni tok i rotore',
      6: 'Složene višestrane raskrsnice',
      7: 'Raskrsnice sa semaforima i signalizacijom',
      8: 'Posebne situacije i iznimke'
    };
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.headerCenter}>
          <Text style={styles.headerTitle}>Raskrsnice</Text>
          <Text style={styles.headerSubtitle}>Lekcije • Prvenstvo Prolaza</Text>
        </View>
      ...
      >
        ...
        <View style={styles.lessonsSection}>
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>Lekcije Raskrsnica</Text>
            <Text style={styles.sectionSubtitle}>Pravila prvenstva prolaza i sigurnog prolaženja</Text>
          </View>
