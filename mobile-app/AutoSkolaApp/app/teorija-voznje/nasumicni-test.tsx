...
export default function NasumicniTestTeorija() {
  ...
  const loadUserStats = async (user_Id: string) => {
    try {
      ...
      const teorijaResults = userResults.rezultati.filter(result => 
        result.tip === 'teorija'
      );

      console.log('🚗 Rezultati teorije:', teorijaResults.length, 'testova');

      let completedTests = teorijaResults.length;
      ...
      console.log('📈 Nova statistika teorije:', newStats);
      setUserStats(newStats);

    } catch (error) {
      console.log('🚨 Greška pri učitavanju statistike teorije:', error);
      setUserStats({
        completedTests: 0,
        correctAnswers: 0,
        totalQuestions: 0,
  ...
  const loadRandomQuestions = async () => {
    try {
      console.log('🔍 Učitavam pitanja za teoriju sa APIja...');
      setIsLoading(true);
      setLoadError(false);
      ...
      const teorijaQuestions = await response.json();
      console.log('✅ Učitano teorija pitanja:', teorijaQuestions.length);
      
      if (!Array.isArray(teorijaQuestions)) {
        throw new Error('API nije vratio listu pitanja');
      }

      if (teorijaQuestions.length === 0) {
        throw new Error('Nema pitanja za teoriju na serveru');
      }

      const shuffledQuestions = [...teorijaQuestions]
        .sort(() => 0.5 - Math.random());
      
      const selectedQuestions = shuffledQuestions.slice(0, 20);
      
      console.log('🎯 Odabrano nasumičnih pitanja za teoriju:', selectedQuestions.length);
      setQuestions(selectedQuestions);
      
    } catch (error) {
      console.error('🚨 Greška pri učitavanju pitanja teorije:', error);
      setLoadError(true);
      setQuestions([]);
    } finally {
      setIsLoading(false);
  ...
  const startTest = () => {
    ...
    console.log('🚀 Pokrećem NASUMIČNI test TEORIJE sa pitanjima:', questions.length);
    
    router.push({
      pathname: "/teorija-voznje/test-screen",
      params: {
        questions: JSON.stringify(questions),
        testType: 'nasumicni-teorija',
        testMode: 'nasumicni',
        userId: userId
      }
    });
  ...
  const handleBackToTests = () => {
    ...
    // Vrati korisnika na nasumične testove teorije
  ...
  return (
    <View style={styles.container}>
      ...
      >
        <TouchableOpacity onPress={() => router.push('/teorija-voznje/nasumicni-test')} style={styles.backButton}>
          <Ionicons name="chevron-back" size={28} color="#FFF" />
        </TouchableOpacity>
        <View style={styles.headerCenter}>
          <Text style={styles.headerTitle}>Nasumični Test</Text>
          <Text style={styles.headerSubtitle}>Teorija Vožnje • Random pitanja</Text>
        </View>
        ...
        >
          ...
          <View style={styles.infoCard}>
            ...
            <View style={styles.infoItem}>
              ...
              <View style={styles.infoText}>
                <Text style={styles.infoTitle}>Teorija Vožnje</Text>
                <Text style={styles.infoDescription}>Sva pitanja su iz oblasti saobraćajnih pravila, znakova i bezbjednosti</Text>
              </View>
            </View>
          ...
          <View style={styles.motivationCard}>
            ...
            <Text style={styles.motivationText}>
              {userStats.completedTests > 0 
                ...
                : 'Ovo je tvoja prva prilika da pokažeš svoje znanje iz teorije vožnje!'
              }
            </Text>
          </View>
        </Animated.View>
          ...
          <Animated.View style={[
            ...
            <View style={styles.modalBody}>
              ...
              <Text style={styles.modalMessage}>
                {testResults?.passed 
                  ? 'Odlično ste savladali test iz teorije vožnje! Nastavite sa vježbanjem.'
