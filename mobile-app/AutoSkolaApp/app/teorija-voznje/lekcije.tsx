...
export default function LekcijeVoznje() {
  ...
  const loadProgressFromAPI = async (user_Id: string) => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/testresults/user/${user_Id}/teorija/lekcijski`
      );
      
      console.log('🌐 API URL:', `${API_BASE_URL}/api/testresults/user/${user_Id}/teorija/lekcijski`);
  ...
  const handleLessonSelect = (lessonNumber: number) => {
    ...
    router.push({
      pathname: `/teorija-voznje/lekcija`,
      params: { 
        lesson: lessonNumber,
        userId: userId
      }
  ...
  const getLessonDescription = (lessonNumber: number) => {
    const descriptions = {
      1: 'Pojmovi, vozila, učesnici u saobraćaju, osnovna pravila',
      2: 'Opasnost, obaveza, zabrana, informativni znakovi',
      3: 'Desna strana, prvenstvo prolaza, brzina kretanja',
      4: 'Pravila raskrsnica, pretjecanje, zaobilazak',
      5: 'Pravila autoputa, brzine, zaustavljanje',
      6: 'Kategorije vozila, dokumentacija, tehnički pregled',
      7: 'Sistemi kočenja, bezbjednosni razmaci, vožnja po kiši',
      8: 'Osvjetljenje, magla, snijeg, klizav kolovoz'
    };
    return descriptions[lessonNumber as keyof typeof descriptions] || '15 pitanja';
  };
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.headerCenter}>
          <Text style={styles.headerTitle}>Teorija Vožnje</Text>
      ...
      >
        ...
        <View style={styles.lessonsSection}>
          <View style={styles.sectionHeader}>
            <Text style={styles.sectionTitle}>Lekcije Teorije Vožnje</Text>
            <Text style={styles.sectionSubtitle}>Priprema za vozački ispit</Text>
          </View>
