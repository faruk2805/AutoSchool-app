...
export default function NasumicniTestPrvaPomoc() {
  ...
  const loadUserStats = async (user_Id: string) => {
    try {
      ...
      const prvaPomocResults = userResults.rezultati.filter(result => 
        result.tip === 'prva_pomoc'
      );

      console.log('🩺 Rezultati prve pomoći:', prvaPomocResults.length, 'testova');

      let completedTests = prvaPomocResults.length;
      ...
      prvaPomocResults.forEach(result => {
        // Koristimo novi format podataka
        totalCorrectAnswers += result.correctCount;
        totalQuestions += result.total;
        totalIncorrectAnswers += (result.total - result.correctCount);
        
        const score = result.score; // score je već number
        totalScore += score;
  ...
  const loadRandomQuestions = async () => {
    try {
      console.log('🔍 Učitavam pitanja za prvu pomoć sa APIja...');
      setIsLoading(true);
      setLoadError(false);
      
      const response = await fetch(`${API_BASE_URL}/api/questions/tip/prva_pomoc`);
      ...
      const prvaPomocQuestions = await response.json();
      console.log('✅ Učitano prva pomoc pitanja:', prvaPomocQuestions.length);
      
      if (!Array.isArray(prvaPomocQuestions)) {
        throw new Error('API nije vratio listu pitanja');
      }

      if (prvaPomocQuestions.length === 0) {
        throw new Error('Nema pitanja za prvu pomoć na serveru');
      }

      const shuffledQuestions = [...prvaPomocQuestions]
        .sort(() => 0.5 - Math.random());
      
      const selectedQuestions = shuffledQuestions.slice(0, 20);
      
      console.log('🎯 Odabrano nasumičnih pitanja:', selectedQuestions.length);
  ...
  const startTest = () => {
    ...
    console.log('🚀 Pokrećem NASUMIČNI test sa pitanjima:', questions.length);
    
    router.push({
      pathname: "/prva-pomoc/test-screen",
      params: {
        questions: JSON.stringify(questions),
        testType: 'nasumicni-prva-pomoc',
        testMode: 'nasumicni',
        userId: userId
      }
    });
  ...
  const handleBackToTests = () => {
    ...
    // Vrati korisnika na nasumične testove prve pomoći
  ...
  return (
    <View style={styles.container}>
      ...
      >
        <TouchableOpacity onPress={() => router.push('/prva-pomoc/nasumicni-test')} style={styles.backButton}>
          <Ionicons name="chevron-back" size={28} color="#FFF" />
        </TouchableOpacity>
        <View style={styles.headerCenter}>
          <Text style={styles.headerTitle}>Nasumični Test</Text>
          <Text style={styles.headerSubtitle}>Prva Pomoc • Random pitanja</Text>
        </View>
        ...
        >
          ...
          <View style={styles.infoCard}>
            ...
            <View style={styles.infoItem}>
              ...
              <View style={styles.infoText}>
                <Text style={styles.infoTitle}>Prva Pomoc</Text>
                <Text style={styles.infoDescription}>Sva pitanja su iz oblasti prve pomoći i hitnih medicinskih intervencija</Text>
              </View>
            </View>
          ...
          <View style={styles.motivationCard}>
            ...
            <Text style={styles.motivationText}>
              {userStats.completedTests > 0 
                ...
                : 'Ovo je tvoja prva prilika da pokažeš svoje znanje iz prve pomoći!'
              }
            </Text>
          </View>
        </Animated.View>
          ...
          <Animated.View style={[
            ...
            <View style={styles.modalBody}>
              ...
              <Text style={styles.modalMessage}>
                {testResults?.passed 
                  ? 'Odlično ste savladali test iz prve pomoći! Nastavite sa vježbanjem.'
