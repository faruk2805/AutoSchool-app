...
export default function LekcijaPrvaPomoc() {
  ...
  useEffect(() => {
    console.log('📖 Učitavam lekciju:', lessonNumber);
    loadUserData();
    loadQuestions();
  }, [lessonNumber]);
  ...
  const loadQuestions = async () => {
    try {
      console.log('📚 Učitavam pitanja za lekciju:', lessonNumber);
      setIsLoading(true);
      ...
      console.log('✅ Učitano svih pitanja:', allQuestions.length);
      ...
      console.log(`📖 Lekcija ${lessonNumber}: pitanja ${startIndex + 1}-${endIndex}`);
  ...
  const sendResultsToBackend = async (correctCount: number, totalQuestions: number, score: number): Promise<boolean> => {
    try {
      ...
      // ISPRAVLJENO: Pravi payload prema tvom backend modelu
      const testResult = {
        userId: userId,
        tip: 'prva_pomoc',
        subTip: 'lekcijski',
        odgovori: odgovori.filter(odgovor => odgovor.odgovor !== "Nije odgovoreno") // Filtriraj neodgovorena
      };

      console.log('📤 Šaljem rezultate na backend:', testResult);

      // ISPRAVLJENO: Koristi ispravan endpoint
      ...
      console.log('✅ Rezultati uspješno poslani:', result);
      return true;
  ...
  const saveLessonProgress = async (correctCount: number, totalQuestions: number, score: number, passed: boolean) => {
    try {
      const progressData = await AsyncStorage.getItem('prvaPomocProgress');
      ...
      // Ako lekcija ne postoji u progresu, dodaj je
      ...
      await AsyncStorage.setItem('prvaPomocProgress', JSON.stringify(updatedProgress));
      console.log('💾 Progres lekcije sačuvan');
  ...
  const calculateResults = async () => {
    ...
    console.log('📊 Rezultat:', { correctCount, total: questions.length, score, passed });

    // Pošalji rezultate na backend
    const backendSuccess = await sendResultsToBackend(correctCount, questions.length, score);

    // Sačuvaj progres lokalno SAMO ako je backend uspješan
    if (backendSuccess) {
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.header}>
          <TouchableOpacity onPress={() => router.push('/Prva_pomoc/lekcije')} style={styles.backButton}>
