...
export default function ProfileScreen() {
  ...
  const loadUserData = async () => {
    try {
      ...
      console.log('User documents:', userData.documents); // Debug documents
      setUser(userData);
  ...
  const loadTestResults = async (userId: string) => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/testresults/user/${userId}`);
      ...
      const sortedResults = userResults.rezultati.sort((a, b) => 
  ...
  const getStatusText = (status: User['status']) => {
    if (!status || !status.polozio) {
      return 'Podaci o statusu nisu dostupni';
    }
    
    const passed = [];
    if (status.polozio.teoriju) passed.push('Teoriju');
    if (status.polozio.prvuPomoc) passed.push('Prvu Pomoc');
    if (status.polozio.voznju) passed.push('Voznju');
    
    return passed.length > 0 ? `Položeno: ${passed.join(', ')}` : 'U toku obuke';
  };
  ...
  const getDocumentInfo = (documents: any) => {
    ...
    console.log('Documents:', documents); // Debug
    
    // Pokušaj pronaći JMBG u dokumentima
    const jmbgDoc = documents.find((doc: any) => 
  ...
  const getDocumentDetails = (documents: any) => {
    ...
    documents.forEach((doc: any, index: number) => {
      const docName = doc.name || doc.type || doc.title || `Dokument ${index + 1}`;
  ...
  const displayedResults = showMoreResults ? testResults : testResults.slice(0, 3);
          ...
          <View style={styles.headerContent}>
            <View style={styles.welcomeSection}>
              ...
              <View style={styles.welcomeTextContainer}>
                ...
                <Text style={styles.roleText}>{user.role === 'kandidat' ? 'Kandidat' : user.role}</Text>
        ...
        <View style={styles.resultsSection}>
          ...
          {loadingResults ? (
            <View style={styles.loadingResults}>
              ...
              <Text style={styles.loadingResultsText}>Učitavanje rezultata...</Text>
            </View>
              ...
              {displayedResults.map((result, index) => (
                <View key={result._id} style={styles.resultCard}>
                  <View style={styles.resultHeader}>
                    ...
                    <View style={[
                      styles.resultStatus,
                      { backgroundColor: result.passed ? '#E8F5E8' : '#FFE6E6' }
                    ]}>
                      <Ionicons 
              ...
              {testResults.length > 3 && (
                ...
                >
                  <Text style={styles.showMoreText}>
                    {showMoreResults ? 'Prikaži manje' : `Prikaži više (${testResults.length - 3})`}
                  </Text>
                  <Ionicons 
