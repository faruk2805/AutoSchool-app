...
export default function VoznjeAdmin() {
  ...
  const mockCandidates = [
    ...
    },
    {
      ...
      status: {
        teorijaPrvaPomoc: true,
        polozio: { teoriju: false, prvuPomoc: true, voznju: false }
      }
    },
    {
      _id: 'mock_candidate_6',
      name: 'Haris',
      surname: 'Delić',
      email: 'haris.delic@example.com',
      phone: '+38763344556',
      status: {
        teorijaPrvaPomoc: false,
        polozio: { teoriju: false, prvuPomoc: false, voznju: false }
  ...
  // Mock podaci za instruktore
  const mockInstructors = [
    {
      _id: 'mock_instructor_1',
      name: 'Amir',
      surname: 'Hodžić',
    ...
    {
      _id: 'mock_instructor_3',
      name: 'Dario',
      surname: 'Marić',
      email: 'dario.maric@example.com',
      phone: '+38763333444',
      status: 'vacation'
    }
  ];
  ...
  // Bogati mock podaci za vožnje - ažurirani za trenutni mjesec
  ...
  const mockDrivingSessions = [
    ...
    // Prošle sedmice - završene vožnje
    {
      _id: 'mock_session_11',
      kandidat: mockCandidates[1],
      instruktor: mockInstructors[1],
  ...
  const fetchWithTimeout = async () => {
    try {
      ...
      const sessionsPromise = fetch('http://localhost:5000/api/driving', {
        headers: {
      ...
      if (response.ok) {
        ...
        if (apiSessions.length > 0) {
          setDrivingSessions(apiSessions);
        }
      }
    } catch (error) {
  ...
  const handleAddDriving = async (drivingData) => {
    try {
      const newSession = {
        ...
        instruktor: instructors.find(i => i._id === drivingData.instruktor),
        vozilo: vehicles.find(v => v._id === drivingData.vozilo),
        datum: drivingData.datum,
        vrijeme: drivingData.vrijeme,
        trajanje: drivingData.trajanje,
        status: 'zakazana',
      ...
      setSnackbar({ open: true, message: 'Greška pri zakazivanju vožnje', severity: 'error' });
    }
  };
  ...
  const handleCancelSession = async (sessionId) => {
    try {
      ...
      setSnackbar({ open: true, message: 'Greška pri otkazivanju vožnje', severity: 'error' });
    }
  };

  const handleCancelDay = async (cancelData) => {
    try {
      const { date, instruktorId, razlog } = cancelData;
  ...
  const handleEnterResult = async (resultData) => {
    try {
      ...
      setSnackbar({ open: true, message: 'Greška pri unosu rezultata', severity: 'error' });
    }
  };
  ...
  const filteredSessions = drivingSessions.filter(session => {
    const matchesInstructor = filterInstructor === "all" || session.instruktor._id === filterInstructor;
